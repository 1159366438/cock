...
const routes = [
  ...
  {
    path: ROUTE_PATHS.PAGES.HOME,
    redirect: ROUTE_PATHS.PAGES.PUNCH
  },
  {
    path: ROUTE_PATHS.PAGES.PUNCH,
    name: ROUTE_NAMES.PAGES.PUNCH,
    component: PunchPage,
    meta: { title: PAGE_NAMES.PUNCH() }
  },
  {
    path: ROUTE_PATHS.PAGES.RECORD,
    name: ROUTE_NAMES.PAGES.RECORD,
...
router.beforeEach((to, _, next) => {
  ...
  if (to.matched.some(record => record.meta?.requiresAuth !== BOOLEAN_CONSTANTS.FALSE) && to.path !== ROUTE_PATHS.AUTH.LOGIN) {
    // 这里应该检查用户的认证状态
